...
async def get_redis():
    if ("redis_pubsub" or "redis_connection") not in g:
        connection = await aioredis.from_url('redis://ec2-3-250-153-214.eu-west-1.compute.amazonaws.com',port=6379,username='default',password=current_app.config['REDIS'])
